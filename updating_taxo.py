...
taxonomy_list = open(taxa_csv_path, "w") # Generates new taxonomy list in csv format
taxonomy_list.write("list_original_name,kingdom,phyla,class,order,family,genus,species,strain\n") # Adds columns to this
...
for strain in strain_list: # loops over the stains on list 
  ...
  #  if split_name[-2] == "sp." or split_name[-2] == "bacterium":
   ...
   #     new_name = species
    ...
    taxonomy_list.write(strain_stripped + ',' + superkingdom + "," + phylum + "," + clazz + "," + order + "," + family + "," + genus + "," + species + "," + strain_val + "\n")
...
act_name_change_path = sys.argv[2] + "/" + "actual_name_changes.csv"  # For the path of the actual name change file
...
readme_file_path = sys.argv[2] + "/" + "README"
readme_file = open(readme_file_path, "w")
readme_file.write("Taxonomy info produced in this directory from taxonomy_updater script" 
+  "\n" + "Utilises ete3 toolkit/NCBI taxonomic database to find lineage and update taxonomy names."
+ "\n" + "Contains four files:"  
+ "\n" + "failed_taxos.txt - any names that could not be identified"
+  "\n" + "identified_taxonomies.csv - lineages of all taxonomies named"
+ "\n" + "name_changes_summary.csv - all old names and new names, including unchanged"
+ "\n" + "actual_name_changes.csv - list of names that are actually changed")
